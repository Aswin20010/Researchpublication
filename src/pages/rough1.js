...
export const Search = () => {
  ...
  const nameFn = async () => {
    ...
    const q = query(collection(db, "UserInfo"), where("UserName", "==", name));
    const userSnapshot = await getDocs(q);
    userSnapshot.forEach(async (doc) => {
      const userId = doc.id;
      const q1 = query(
        collection(db, "ResearchInfo"),
        where("Author", "==", userId)
      );
      const researchSnapshot = await getDocs(q1);
      researchSnapshot.forEach(async (ResearchInfoItem) => {
        const researchId = ResearchInfoItem.id;
        const q2 = query(
        ...
        journalSnapshot.forEach(async (JournalInfoItem) => {
          records.push({
            id: JournalInfoItem.id,
            value: JournalInfoItem.data(),
          });
        });
        setData(records);
      });
